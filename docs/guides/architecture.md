...
- `Tests/SuttoE2ETests` drives the real app bundle from the outside (injected shortcut → panel → actual window movement). It depends on SuttoDomain only — shared constants and frame math — and observes the app like an external tool: through the Accessibility API and through the app's documented persistence contract (the collection files and preference keys shared with the GNOME version), never through SuttoInfra/SuttoUI internals.
